...
test_that("get_i18n rejects invalid language", {
  ...
  expect_error(nemeton:::get_i18n("invalid"))
  expect_error(nemeton:::get_i18n(""))
})
...
test_that("print.nemeton_i18n displays translator information", {
  ...
  output <- capture.output(nemeton:::print.nemeton_i18n(i18n))
...
test_that("print.nemeton_i18n returns object invisibly", {
  ...
  result <- capture.output(returned <- nemeton:::print.nemeton_i18n(i18n))
...
test_that("print.nemeton_i18n works for both languages", {
  ...
  # Call the print method directly
  output_fr <- capture.output(nemeton:::print.nemeton_i18n(i18n_fr))
  output_en <- capture.output(nemeton:::print.nemeton_i18n(i18n_en))
...
test_that("translate_task_message handles NULL and empty string", {
  ...
  result_null <- nemeton:::translate_task_message(NULL, i18n)
  expect_equal(result_null, "")

  result_empty <- nemeton:::translate_task_message("", i18n)
...
test_that("translate_task_message handles special task keywords", {
  ...
  # Test download_start
  result_fr <- nemeton:::translate_task_message("download_start", i18n_fr)
  expect_true(nchar(result_fr) > 0)

  result_en <- nemeton:::translate_task_message("download_start", i18n_en)
  expect_true(nchar(result_en) > 0)

  # Test compute_start
  result <- nemeton:::translate_task_message("compute_start", i18n_en)
  expect_true(nchar(result) > 0)

  # Test complete
  result <- nemeton:::translate_task_message("complete", i18n_en)
  expect_true(nchar(result) > 0)

  # Test error
  result <- nemeton:::translate_task_message("error", i18n_en)
  expect_true(nchar(result) > 0)

  # Test resuming
  result <- nemeton:::translate_task_message("resuming", i18n_en)
  expect_true(nchar(result) > 0)
})
...
test_that("translate_task_message handles download_complete", {
  ...
  result_fr <- nemeton:::translate_task_message("download_complete", i18n_fr)
  expect_true(grepl("termin\u00e9", result_fr, ignore.case = TRUE))

  result_en <- nemeton:::translate_task_message("download_complete", i18n_en)
...
test_that("translate_task_message handles download:source_key format", {
  ...
  result_fr <- nemeton:::translate_task_message("download:source_ndvi", i18n_fr)
  expect_true(grepl("T\u00e9l\u00e9chargement", result_fr))

  result_en <- nemeton:::translate_task_message("download:source_ndvi", i18n_en)
  expect_true(grepl("Downloading", result_en))

  # Test with source_dem
  result <- nemeton:::translate_task_message("download:source_dem", i18n_en)
...
test_that("translate_task_message returns task as-is for unknown format", {
  ...
  result <- nemeton:::translate_task_message("unknown_task_format", i18n)
  expect_equal(result, "unknown_task_format")

  result2 <- nemeton:::translate_task_message("some_random_message", i18n)
...
test_that("export_translations_json creates output directory if needed", {
  ...
  nemeton:::export_translations_json(temp_dir)
...
test_that("export_translations_json returns NULL invisibly", {
  ...
  result <- nemeton:::export_translations_json(temp_dir)
...
test_that("export_translations_json works with existing directory", {
  ...
  expect_no_error(nemeton:::export_translations_json(temp_dir))
