...
test_that("eobs_downscale dégradé -> status propagé, meta.json écrit, cache NA", {
  ...
  paths <- nemetonshiny:::.regen_context_raster_paths(pp, "tx")
...
test_that("vue rr appelle eobs_downscale(var=rr) ; vue bivariate appelle le bivarié", {
  skip_if_not_installed("sf")
  u <- .ctx_units(2); pp <- withr::local_tempdir()
  seen <- new.env(); seen$var <- NULL; seen$biv <- FALSE
  testthat::local_mocked_bindings(
    regen_eobs_cached_nc = function(project_path, var = "tx") "FAKE.nc",
    .package = "nemetonshiny")
  testthat::local_mocked_bindings(
    load_eobs_source = function(aoi, var, nc, ...) paste0(var, "_STACK"),
    eobs_downscale = function(var, ...) { seen$var <- var
      list(raster = NULL, meta = list(status = "insufficient_data", reason = "eobs_downscale_no_dem")) },
    eobs_downscale_bivariate = function(tx, rr, ...) { seen$biv <- TRUE
      list(raster = NULL, meta = list(status = "insufficient_data", reason = "eobs_downscale_no_dem")) },
    .package = "nemeton")

  nemetonshiny:::run_regeneration_context_raster(u, pp, view = "rr")
  expect_identical(seen$var, "rr")
  expect_false(seen$biv)

  seen$var <- NULL
  nemetonshiny:::run_regeneration_context_raster(u, pp, view = "bivariate")
  expect_true(seen$biv)                      # le bivarié croise tx × rr
})

test_that("vue rr/bivariate sans série rr -> status need_rr", {
  skip_if_not_installed("sf")
  u <- .ctx_units(1); pp <- withr::local_tempdir()
  testthat::local_mocked_bindings(
    regen_eobs_cached_nc = function(project_path, var = "tx") if (var == "rr") NULL else "FAKE.nc",
    .package = "nemetonshiny")
  testthat::local_mocked_bindings(
    load_eobs_source = function(aoi, var, nc, ...) paste0(var, "_STACK"),
    .package = "nemeton")

  expect_identical(nemetonshiny:::run_regeneration_context_raster(u, pp, view = "rr")$meta$status, "need_rr")
  expect_identical(nemetonshiny:::run_regeneration_context_raster(u, pp, view = "bivariate")$meta$status, "need_rr")
})

test_that("les 3 vues ont des chemins de cache distincts", {
  pp <- withr::local_tempdir()
  ptx <- nemetonshiny:::.regen_context_raster_paths(pp, "tx")$tif
  prr <- nemetonshiny:::.regen_context_raster_paths(pp, "rr")$tif
  pbi <- nemetonshiny:::.regen_context_raster_paths(pp, "bivariate")$tif
  expect_equal(length(unique(c(ptx, prr, pbi))), 3L)
})
...
test_that("regeneration_context_cached relit .tif + meta.json (fast-path)", {
  ...
  cached <- nemetonshiny:::regeneration_context_cached(pp, "tx")
