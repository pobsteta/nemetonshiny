...
prepare_report_data <- function(project, family_scores, language,
  ...
  # Build UG composition table (for cadastral annex)
  ug_composition <- NULL
  if (has_ugs) {
    ug_composition <- tryCatch({
      ugs_df <- project$ugs
      lapply(seq_len(nrow(ugs_df)), function(i) {
        uid <- ugs_df$ug_id[i]
        refs <- ug_cadastral_refs(project, uid)
        list(
          label = ugs_df$label[i],
          groupe = ugs_df$groupe[i],
          surface_ha = round(ug_surface(project, uid) / 10000, 2),
          cadastral_refs = if (nrow(refs) > 0) {
  ...
  list(
    ...
    labels = list(
      ...
      ug_section_title = if (language == "fr") "Unit\u00e9s de Gestion" else "Management Units",
      ug_composition_title = if (language == "fr") "Composition cadastrale" else "Cadastral Composition"
    )
  )
}
