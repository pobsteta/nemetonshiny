...
TRANSLATIONS <- list(
  ...
  acc_buffer = list(fr = "Zone tampon autour de la for\u00eat (m)", en = "Buffer around the forest (m)"),
  acc_buffer_help = list(fr = "\u00c9largit l'emprise du MNT et de la desserte au-del\u00e0 de la for\u00eat : l'acc\u00e8s vient des routes ext\u00e9rieures. 0 = emprise for\u00eat seule.", en = "Widens the DEM and road-network extent beyond the forest: access comes from outside roads. 0 = forest extent only."),
  acc_opacity = list(fr = "Opacit\u00e9 du raster", en = "Raster opacity"),
  acc_no_result_yet = list(fr = "Aucun raster calcul\u00e9. Lancez l'analyse pour afficher les couches.", en = "No raster computed yet. Run the analysis to display the layers."),
  acc_cache_loaded_fmt = list(fr = "R\u00e9sultats d'accessibilit\u00e9 pr\u00e9c\u00e9dents charg\u00e9s (%d couche(s)).", en = "Previous accessibility results loaded (%d layer(s))."),
  ...
  acc_correct_section = list(fr = "Desserte LiDAR (NDP 1)", en = "LiDAR road network (NDP 1)"),
  acc_correct_run = list(fr = "Corriger la desserte au LiDAR", en = "Correct the road network with LiDAR"),
  acc_correct_running = list(fr = "Correction LiDAR de la desserte en cours\u2026", en = "LiDAR road-network correction running\u2026"),
  acc_correct_busy = list(fr = "Correction LiDAR en cours \u2014 patientez avant de lancer l'analyse.", en = "LiDAR correction running \u2014 wait before launching the analysis."),
  acc_analysis_busy = list(fr = "Analyse en cours \u2014 patientez avant de corriger la desserte.", en = "Analysis running \u2014 wait before correcting the road network."),
  acc_ndp1_duration_note = list(fr = "Calcul long (~2-3 h) et gourmand en m\u00e9moire : la desserte est recal\u00e9e sur sa vraie position LiDAR, les tron\u00e7ons fant\u00f4mes retir\u00e9s et la largeur mesur\u00e9e. Fait UNE fois, puis r\u00e9utilis\u00e9 par les runs moteurs (case ci-dessous). Le r\u00e9sultat est \u00e9crit sur disque m\u00eame si le navigateur se ferme.", en = "Long (~2-3 h) and memory-heavy: the network is re-aligned to its true LiDAR position, phantom segments removed and width measured. Done ONCE, then reused by engine runs (checkbox below). The result is written to disk even if the browser closes."),
  acc_correct_done_fmt = list(fr = "Desserte corrig\u00e9e : %d tron\u00e7ons (%d BD TOPO conserv\u00e9s + %d ajout\u00e9s d'OSM).", en = "Corrected network: %d segments (%d BD TOPO kept + %d added from OSM)."),
  acc_correct_osm_indispo = list(fr = "OSM indisponible : desserte BD TOPO seule, sans compl\u00e9ment.", en = "OSM unavailable: BD TOPO network only, no complement."),
  ...
  acc_compare_relief_hillshade = list(fr = "Fond : relief ombr\u00e9 multi-directionnel.", en = "Background: multi-directional shaded relief."),
  acc_cvat_prebuild_running = list(fr = "Fond relief CVAT : calcul unique sur l'emprise (~1 min)", en = "CVAT relief background: one-off computation over the extent (~1 min)"),
  acc_cvat_prebuild_done = list(fr = "Fond relief CVAT pr\u00eat.", en = "CVAT relief background ready."),
  acc_cvat_prebuild_failed = list(fr = "Fond relief CVAT indisponible (calcul\u00e9 \u00e0 la demande).", en = "CVAT relief background unavailable (computed on demand)."),
  acc_compare_building_relief = list(fr = "Calcul du fond relief en cours\u2026", en = "Computing the relief background\u2026"),
  acc_compare_legend_origine = list(fr = "BD TOPO (classe)", en = "BD TOPO (class)"),
  acc_compare_legend_corrigee = list(fr = "Correction LiDAR", en = "LiDAR correction"),
  acc_compare_non_mesure = list(fr = "Non mesur\u00e9 (hors couverture LiDAR)", en = "Not measured (outside LiDAR coverage)"),
  acc_desserte_route = list(fr = "Route", en = "Road"),
  acc_desserte_piste = list(fr = "Piste", en = "Track"),
  acc_desserte_reseau_public = list(fr = "R\u00e9seau public", en = "Public network"),
  acc_desserte_hors = list(fr = "Hors desserte", en = "Outside road network"),
  acc_desserte_en_service = list(fr = "En service", en = "In service"),
  acc_desserte_trouee = list(fr = "Trou\u00e9e sans route", en = "Clearing without road"),
  acc_desserte_abandonnee = list(fr = "Abandonn\u00e9e (recolonis\u00e9e)", en = "Abandoned (recolonised)"),
  acc_desserte_sans_signal = list(fr = "Aucun signal mesur\u00e9", en = "No signal measured"),
  ...
  acc_desserte_ndp0_badge = list(fr = "Desserte corrig\u00e9e demand\u00e9e mais absente : repli sur la desserte brute.", en = "Corrected network requested but missing: fell back to the raw network."),
  ...
  # Profil en travers au clic (spec 030 cote foretaccess). Les libelles de bords
  # traduisent le vocabulaire TECHNIQUE rendu par le coeur (`bords$type`) : ne
  # pas renommer les cles, elles sont construites par concatenation.
  profil_titre = list(fr = "Profil en travers du tron\u00e7on",
                      en = "Road segment cross-section"),
  profil_calcul = list(fr = "Lecture du nuage LiDAR et coupe du tron\u00e7on\u2026",
                       en = "Reading the LiDAR cloud and cutting the section\u2026"),
  profil_axe_x = list(fr = "Distance \u00e0 l'axe (m)", en = "Distance from axis (m)"),
  profil_axe_y = list(fr = "Hauteur / sol (m)", en = "Height above ground (m)"),
  profil_legend_hauteur = list(fr = "Hauteur (m)", en = "Height (m)"),
  profil_serie_vegetation = list(fr = "Points hors sol", en = "Above-ground points"),
  profil_serie_sol = list(fr = "Points sol", en = "Ground points"),
  profil_serie_terrain = list(fr = "Profil du terrain", en = "Ground profile"),
  profil_serie_chaussee = list(fr = "Chauss\u00e9e ajust\u00e9e", en = "Fitted crown"),
  profil_serie_axe = list(fr = "Axe du tron\u00e7on", en = "Segment axis"),
  profil_bord_drivable = list(fr = "Chauss\u00e9e roulable", en = "Drivable"),
  profil_bord_road = list(fr = "Plateforme", en = "Road"),
  profil_bord_rescue = list(fr = "Bande de secours", en = "Rescue"),
  profil_bord_right_of_way = list(fr = "Emprise", en = "Right of way"),
  profil_bord_shoulder = list(fr = "Accotement", en = "Shoulder"),
  profil_station_fmt = list(fr = "Cha\u00eenage %.0f m sur le tron\u00e7on \u2014 %d points LiDAR",
                            en = "Chainage %.0f m along the segment \u2014 %d LiDAR points"),
  profil_no_project = list(fr = "Aucun projet ouvert.", en = "No project open."),
  profil_no_desserte = list(fr = "Aucune desserte corrig\u00e9e : lancez d'abord la correction LiDAR.",
                            en = "No corrected road network: run the LiDAR correction first."),
  profil_no_lidar = list(fr = "Aucun nuage LiDAR dans le projet : le profil en travers en a besoin.",
                         en = "No LiDAR cloud in the project: the cross-section needs one."),
  profil_no_mnt = list(fr = "Aucun MNT disponible pour ce projet.",
                       en = "No DTM available for this project."),
  profil_no_segment = list(fr = "Aucun tron\u00e7on \u00e0 moins de 25 m du point cliqu\u00e9.",
                           en = "No segment within 25 m of the clicked point."),
  profil_failed = list(fr = "Le calcul du profil en travers a \u00e9chou\u00e9.",
                       en = "The cross-section computation failed."),
  profil_bad_point = list(fr = "Point cliqu\u00e9 illisible.", en = "Unreadable clicked point."),
  ...
  dess_engine_steiner = list(fr = "Steiner (arbre couvrant) \u2014 bien moins cher, bien plus long", en = "Steiner (spanning tree) \u2014 far cheaper, far slower"),
  dess_slow_help = list(fr = "Calcul tr\u00e8s long : le glouton trace une route par parcelle sur une grille \u00e0 5 m. Le temps d\u00e9pend de la SURFACE de l'emprise (parcelles + tampon), pas du nombre de parcelles : comptez ~10 min pour 300 000 cellules, et plusieurs dizaines de minutes au-del\u00e0 du million. Le calcul continue en arri\u00e8re-plan si vous changez d'onglet.", en = "Very long computation: greedy traces one road per parcel on a 5 m grid. Time depends on the AREA of the extent (parcels + buffer), not on the parcel count: allow ~10 min for 300,000 cells, and several tens of minutes beyond one million. The computation keeps running in the background if you switch tabs."),
  dess_phase_mnt = list(fr = "Acquisition du MNT", en = "Fetching the DEM"),
  dess_phase_desserte = list(fr = "Acquisition de la desserte existante", en = "Fetching the existing road network"),
  dess_phase_foret = list(fr = "Acquisition du masque for\u00eat", en = "Fetching the forest mask"),
  dess_phase_preprocess = list(fr = "Pr\u00e9traitement (pente, franchissabilit\u00e9)", en = "Preprocessing (slope, trafficability)"),
  dess_phase_cout = list(fr = "Surface de co\u00fbt de construction", en = "Construction-cost surface"),
  dess_phase_moteur = list(fr = "Moteur glouton (\u00e9tape la plus longue)", en = "Greedy engine (longest step)"),
  ...
  dess_skidding = list(fr = "Distance de d\u00e9bardage (m)", en = "Skidding distance (m)"),
  desserte_core_trop_ancien = list(fr = "Le c\u0153ur foretaccess install\u00e9 ne sait pas tarifer le terrassement ni plafonner la pente constructible (spec 029). Mettre \u00e0 jour foretaccess, ou revenir au bar\u00e8me avec le plafond par d\u00e9faut.", en = "The installed foretaccess core cannot price earthwork nor cap the constructible slope (spec 029). Update foretaccess, or fall back to the step scale with the default ceiling."),
  dess_methode_pente = list(fr = "Tarification de la pente", en = "Slope pricing"),
  dess_methode_bareme = list(fr = "Bar\u00e8me", en = "Step scale"),
  dess_methode_terrassement = list(fr = "Terrassement", en = "Earthwork"),
  dess_methode_pente_help = list(fr = "Le terrassement chiffre un volume de terre ; le bar\u00e8me applique une grille par classe de pente. Les deux donnent des co\u00fbts m\u00e9dians comparables, mais des trac\u00e9s diff\u00e9rents \u00e0 plus de la moiti\u00e9. Le bar\u00e8me est aveugle \u00e0 la largeur de plateforme.", en = "Earthwork prices a volume of soil; the step scale applies a grid by slope class. Both yield comparable median costs, but more than half of the routes differ. The step scale is blind to platform width."),
  dess_largeur = list(fr = "Largeur de plateforme (m)", en = "Platform width (m)"),
  dess_largeur_help = list(fr = "Largeur vis\u00e9e par le gestionnaire. Sans effet avec le bar\u00e8me : le volume terrass\u00e9 cro\u00eet comme son carr\u00e9, ce que le bar\u00e8me ne sait pas repr\u00e9senter.", en = "Width targeted by the manager. No effect with the step scale: earthwork volume grows as its square, which the step scale cannot represent."),
  dess_pente_max = list(fr = "Pente maximale constructible (%)", en = "Maximum constructible slope (%)"),
  dess_pente_max_help = list(fr = "Au-del\u00e0 de ce seuil, aucune route n\u2019est trac\u00e9e. L\u2019augmenter ouvre du terrain que le bar\u00e8me refusait : le co\u00fbt y est tr\u00e8s \u00e9lev\u00e9 mais FINI, donc dissuasif et non interdit \u2014 dans un corridor sans alternative, le solveur le prendra.", en = "Above this threshold no road is traced. Raising it opens terrain the step scale refused: the cost there is very high but FINITE, hence deterrent rather than forbidden \u2014 in a corridor with no alternative, the solver will take it."),
  dess_skidding_help = list(fr = "Une zone \u00e0 moins de cette distance d'une route est consid\u00e9r\u00e9e comme DESSERVIE : aucune route n'y sera cr\u00e9\u00e9e. Ce param\u00e8tre change le r\u00e9sultat, pas seulement la dur\u00e9e \u2014 plus il est grand, moins il y a de routes \u00e0 construire. \u00c0 0, le moteur trace depuis chaque cellule de parcelle : r\u00e9seau sur-connect\u00e9 et calcul interminable.", en = "An area within this distance of a road counts as SERVED: no road will be created there. This parameter changes the result, not just the duration \u2014 the larger it is, the fewer roads to build. At 0 the engine traces from every parcel cell: over-connected network and endless computation."),
  ...
  dess_badge_routes = list(fr = "Routes cr\u00e9\u00e9es", en = "Roads created"),
  ...
  dess_integrite_run = list(fr = "Contr\u00f4ler l'int\u00e9grit\u00e9", en = "Check integrity"),
  dess_integrite_running = list(fr = "Contr\u00f4le d'int\u00e9grit\u00e9 du r\u00e9seau\u2026", en = "Checking network integrity\u2026"),
  dess_integrite_done = list(fr = "Contr\u00f4le d'int\u00e9grit\u00e9 termin\u00e9.", en = "Integrity check complete."),
  dess_integrite_hint = list(fr = "Lancez le contr\u00f4le pour afficher les infractions et les composants isol\u00e9s.", en = "Run the check to display violations and isolated components."),
  desserte_integrite_no_dessertr = list(fr = "Paquet dessertR absent : contr\u00f4le d'int\u00e9grit\u00e9 indisponible.", en = "dessertR package missing: integrity check unavailable."),
  desserte_integrite_no_reseau = list(fr = "Aucun r\u00e9seau \u00e0 contr\u00f4ler : lancez d'abord la g\u00e9n\u00e9ration de la desserte.", en = "No network to check: run the road-network generation first."),
  desserte_integrite_failed = list(fr = "Le contr\u00f4le d'int\u00e9grit\u00e9 a \u00e9chou\u00e9.", en = "The integrity check failed."),
  dess_optim_title = list(fr = "Optimisation du r\u00e9seau", en = "Network optimisation"),
  dess_optim_intro = list(fr = "Rejoue plusieurs constructions et retient la moins co\u00fbteuse. Chaque essai est une construction compl\u00e8te : comptez un peu plus que le calcul de desserte lui-m\u00eame, pour un gain de l'ordre de 10 % sur le co\u00fbt.", en = "Replays several builds and keeps the cheapest. Each trial is a full build: allow slightly more than the network creation itself, for a cost gain of around 10%."),
  dess_optim_strategie = list(fr = "Strat\u00e9gie", en = "Strategy"),
  dess_optim_multistart = list(fr = "Multi-d\u00e9parts", en = "Multistart"),
  dess_optim_recuit = list(fr = "Recuit simul\u00e9", en = "Simulated annealing"),
  dess_optim_riprute = list(fr = "Rip-up & reroute", en = "Rip-up & reroute"),
  dess_optim_n_start = list(fr = "Nombre d'essais", en = "Number of trials"),
  dess_optim_run = list(fr = "Optimiser le r\u00e9seau", en = "Optimise the network"),
  dess_optim_running = list(fr = "Optimisation du r\u00e9seau en cours\u2026", en = "Optimising the network\u2026"),
  dess_optim_done = list(fr = "Optimisation termin\u00e9e.", en = "Optimisation complete."),
  dess_optim_hint = list(fr = "Lancez l'optimisation pour comparer son co\u00fbt \u00e0 celui du r\u00e9seau courant.", en = "Run the optimisation to compare its cost with the current network."),
  desserte_optim_bad_strategie = list(fr = "Strat\u00e9gie d'optimisation inconnue.", en = "Unknown optimisation strategy."),
  desserte_optim_no_entrees = list(fr = "Entr\u00e9es terrain indisponibles pour l'optimisation.", en = "Terrain inputs unavailable for the optimisation."),
  desserte_optim_failed = list(fr = "L'optimisation du r\u00e9seau a \u00e9chou\u00e9.", en = "The network optimisation failed."),
  dess_osm_title = list(fr = "Compl\u00e9ment OpenStreetMap", en = "OpenStreetMap complement"),
  dess_osm_intro = list(fr = "Recherche dans OpenStreetMap les pistes absentes de la BD TOPO, puis les recoupe avec le r\u00e9seau existant. Le recoupement demande une \u00e0 deux minutes ; l'acquisition d\u00e9pend du serveur OpenStreetMap, qui limite le d\u00e9bit et peut faire patienter plusieurs minutes aux heures charg\u00e9es. Le calcul continue en arri\u00e8re-plan.", en = "Looks for tracks missing from BD TOPO in OpenStreetMap, then cross-checks them against the existing network. The cross-check takes a minute or two; acquisition depends on the OpenStreetMap server, which rate-limits and may keep you waiting several minutes at busy times. The computation keeps running in the background."),
  dess_osm_run = list(fr = "Comparer \u00e0 OpenStreetMap", en = "Compare with OpenStreetMap"),
  dess_osm_running = list(fr = "Comparaison OpenStreetMap en cours\u2026", en = "Comparing with OpenStreetMap\u2026"),
  dess_osm_done_fmt = list(fr = "%d tron\u00e7ons OpenStreetMap analys\u00e9s.", en = "%d OpenStreetMap segments analysed."),
  dess_osm_hint = list(fr = "Lancez la comparaison pour rep\u00e9rer les pistes absentes de la BD TOPO.", en = "Run the comparison to spot tracks missing from BD TOPO."),
  desserte_osm_failed = list(fr = "La comparaison OpenStreetMap a \u00e9chou\u00e9.", en = "The OpenStreetMap comparison failed."),
  desserte_osm_empty = list(fr = "Aucune piste OpenStreetMap sur cette emprise.", en = "No OpenStreetMap track on this extent."),
  dess_detect_title = list(fr = "D\u00e9tection de routes non cartographi\u00e9es", en = "Detection of unmapped roads"),
  dess_detect_intro = list(fr = "Cherche dans le relief, et dans le nuage LiDAR quand il est disponible, des routes absentes de la BD TOPO.", en = "Looks for roads missing from BD TOPO in the terrain, and in the LiDAR point cloud when available."),
  dess_detect_warn = list(fr = "Traitement le plus lourd de l'onglet : comptez plusieurs minutes et jusqu'\u00e0 8 Go de m\u00e9moire sur une emprise de ~1 800 ha, davantage avec le nuage LiDAR. Le calcul est refus\u00e9 si la m\u00e9moire disponible est insuffisante.", en = "The heaviest processing in this tab: allow several minutes and up to 8 GB of memory on a ~1,800 ha extent, more with the LiDAR cloud. The run is refused if available memory is insufficient."),
  dess_detect_lidar = list(fr = "Utiliser le nuage LiDAR (plus s\u00fbr, plus lent)", en = "Use the LiDAR point cloud (safer, slower)"),
  dess_detect_run = list(fr = "D\u00e9tecter les routes", en = "Detect roads"),
  dess_detect_running = list(fr = "D\u00e9tection de routes en cours\u2026", en = "Detecting roads\u2026"),
  dess_detect_done_fmt = list(fr = "%d route(s) d\u00e9tect\u00e9e(s) hors BD TOPO.", en = "%d road(s) detected outside BD TOPO."),
  dess_detect_hint = list(fr = "Lancez la d\u00e9tection pour rechercher des routes absentes de la BD TOPO.", en = "Run the detection to look for roads missing from BD TOPO."),
  ...
  dess_done_none_fmt = list(fr = "Aucune route \u00e0 construire : le r\u00e9seau existant dessert d\u00e9j\u00e0 toutes les parcelles \u00e0 %.0f m de d\u00e9bardage.", en = "No road to build: the existing network already serves every parcel within %.0f m of skidding."),
  dess_no_road_needed = list(fr = "Le r\u00e9seau existant suffit pour la distance de d\u00e9bardage retenue. R\u00e9duisez cette distance pour voir quelles routes seraient n\u00e9cessaires.", en = "The existing network is sufficient for the chosen skidding distance. Lower that distance to see which roads would be needed."),
  ...
  action_plan_persist_empty = list(fr = "Plan vide : rien \u00e0 enregistrer en base.", en = "Empty plan: nothing to save to the database."),
  action_plan_db_unavailable = list(fr = "Base de donn\u00e9es non configur\u00e9e.", en = "Database not configured."),
  action_plan_persisted_fmt = list(fr = "Plan d'actions enregistr\u00e9 en base (version %d).", en = "Action plan saved to database (version %d)."),
  ...
  # ----- Tour : 1 step cle par onglet (spec tour v2) -----
  ...
  # v0.51.8 - ligne resume au-dessus du selectInput (vue d'ensemble
  # " N/3 configures : Mistral, OpenAI ").
  ...
  # v0.46.4 - wording honnete du panel Theia config. theia_python_ok
  # remplace l'ancienne reutilisation de theia_status_ready qui
  # mentait sur la dispo des modules Python (cf. fix
  # theia_python_ready : on ne probe plus py_module_available, donc
  # " Python ok " signifie juste " reticulate installe ").
  ...
  # (reticulate + cle) mais load_theia_source() echoue a l'execution.
  ...
  # v0.51.6 - sous-section "cle deja configuree" : Modifier / Supprimer.
  ...
  # v0.52.5 - Etat " orphelin " : la base contient des zones mais
  # aucune n'est rattachee au projet charge. Symptome typique :
  # `helper-monitoring.R` cote coeur a wipe + recree une stub
  # (incident villards 2026-05-31, cf. brief coeur v0.54.0).
  ...
  # v0.61.0 - Cle `monitoring_bands` retiree. NDVI + NBR sont
  # systematiquement telecharges lors du Diagnostic FAST (cablage
  # en dur dans `fast_task$invoke()`), donc le choix utilisateur
  # n'a plus lieu d'etre. Les radios NDVI/NBR des sidebars droits
  # des onglets pilotent l'AFFICHAGE, pas le telechargement.
  # v0.52.13 - Label du radio mono-index (spec 017 nemeton@v0.55.0).
  # L'utilisateur choisit l'indice spectral utilise pour le raster
  # d'alerte FAST (NDMI / NDVI / NBR / NDRE en count/rolling). Le seuil
  # correspondant est pilote par les sliders
  ...
  # v0.52.0 - Vrai cancel cooperatif (nemeton@v0.53.0+). Le clic ecrit
  # `<projet>/data/{fast,fordead}_cancel.flag`. Le worker poll ce
  # fichier entre tuiles (FAST) / entre phases reticulate (FORDEAD)
  # et sort proprement au prochain checkpoint. Le libelle reflete ce
  # vrai cancel ; le toast explique le mecanisme (tuile/phase courante
  # finit, puis stop ; INSERT idempotents conserves).
  ...
  # v0.73.0 (spec 020) - Wording bouton + messages reorientes
  ...
  # v0.73.0 - Anciennes cles `monitoring_register_success` et
  # `_already` (singulier, n_plots) remplacees par
  ...
  # v0.73.0 (spec 020) - Cles specifiques aux 4 strates.
  ...
  # strate, %.1f = hectares, %.0f = pourcentage relatif a la strate
  # `_tot` (toutes essences).
  monitoring_fast_surf_item = list(
    fr = "%s : %.1f ha (%.0f %%)",
    en = "%s: %.1f ha (%.0f%%)"
  ),
  # Variante sans pourcentage (strate `_tot`, reference 100 %).
  ...
  # v0.34.0 - sous-onglets de carte pixel separes par mode :
  # FAST (NDVI/NBR raster, mode quick) vs FORDEAD (mask classifie de
  # deperissement, mode health). Visibilite pilotee cote server par
  ...
  # (le sous-onglet vectoriel homonyme a ete supprime en v0.92.x). Renomm\u00e9
  # " Carte FORDEAD " pour la parit\u00e9 avec " Carte FAST " / " Carte RECONFORT ".
  monitoring_subtab_pixel_map_fordead = list(
    fr = "Carte FORDEAD",
    en = "FORDEAD map"
  ),
  # v0.35.0 - sous-onglets Alertes separes par mode, symetriques aux
  # cartes : Alertes FAST (pixels au-dessus du seuil NDVI/NBR
  # rolling-window) vs Alertes FORDEAD (placettes flaguees par
  # run_fordead_dieback). Visibilite pilotee par input$mode.
  ...
  # v0.53.0 - bandeau d'erreur distinct de " zone saine " : affiche
  # quand `read_fast_alert_raster()` leve une exception OU renvoie
  # NULL (typiquement cache S2 incomplet pour l'indice choisi -
  # courant pour NBR si B12 manque sur beaucoup de scenes).
  ...
  # v0.68.0 - Message NULL specifique a " aucune scene cachee ne
  # porte les bandes de cet indice dans la fenetre " (brief FAST 6
  # cartes nemeton@v0.65.0). Cas typique : NDMI demande B08+B11,
  # une zone sans B11 produit NULL pour NDMI sur cette periode.
  # Le `%s` recoit le nom de l'indice (NDVI/NBR/NDMI).
  ...
  # rather than a logical plot, hence "Coordonnees" / "Coordinates".
  monitoring_fast_alert_popup_coords = list(
    fr = "Coordonn\u00e9es",
    en = "Coordinates"
  ),
  # v0.41.1 - "Severite" est conserve le temps que
  # nemeton::read_fast_alert_raster() tranche entre un score continu et
  # des classes discretes. Si score continu : reformuler en
  ...
  # a addRasterImage(read_fast_alert_raster()). Le module expose un
  # toggle compte / rolling : `count` retourne un entier par pixel
  # (nombre de dates en alerte), `rolling` un continu (magnitude).
  # `legend_title` est utilise pour le mode rolling, `legend_count_title`
  # pour le mode count.
  # v0.45.0 - libelles revus apres tests user villards. "Compte" et
  # "Magnitude" etaient abstraits ; "Frequence" / "Intensite" parlent
  # davantage du sens metier (jours en alerte vs intensite du deficit
  # NDVI/NBR sur la fenetre roulante).
  ...
  # v0.55.0 - Cles utilisees par les toasts `fast_prewarm:*` emis par
  # le coeur (spec 018 nemeton@v0.61.0) consommes par l'observer
  # ingest_progress de mod_monitoring.R. Les noms `fast_mode_*` sont
  # courts car reutilisables hors du contexte " alerts " (Carte FAST
  # pourrait aussi vouloir afficher le libelle du mode si besoin).
  ...
  # v0.70.1 - Toast court (4 s) emis a `fast_prewarm:complete` pour
  # signaler la fin du Diagnostic FAST cote UX. Le toast running
  # `fast_prewarm_progress` (persistent) est retire juste avant.
  # Sans ce signal, l'utilisateur ne savait pas si l'application
  # etait de nouveau disponible (le bouton " Lancer le diagnostic
  # FAST " se reactive automatiquement via l'observer status() mais
  # un signal explicite cote UI rassure).
  ...
  # nemeton@v0.55.0+). Le mask categoriel sort de
  # `nemeton::compute_fast_alert_mask()`. La classe 0 (sain) est
  # rendue transparente - pas de libelle i18n necessaire pour elle.
  ...
  # v0.61.0 - Cle `monitoring_fast_alerts_raster_visible` retiree.
  # Visibilite du raster d'alerte pilotee par le LayersControl
  # Leaflet (entree " Alertes " sous " UGF ").
  ...
  # modal pixel de Carte FAST (Livrable 3 spec 013). Format : %s recoit
  ...
  # Empty state shown tant que le writer coeur n'a pas shippe.
  ...
  # Partie B - selecteur de couche pixel de la Carte FORDEAD.
  ...
  # carte FORDEAD. Affiche la serie CRSWIR observee vs la prediction
  # harmonique du modele FORDEAD, avec marqueur vertical sur la
  # date de 1re anomalie detectee.
  ...
  # v0.61.0 - Cle `monitoring_pixel_map_raster_visible` retiree.
  # Visibilite du raster pilotee par le LayersControl Leaflet
  # (entree `"NDVI/NBR"`). Seul le slider d'opacite reste.
  ...
  # "cache absent" (existant) de "cache present mais 0 obs en DB"
  # (typique 403 SAS-token-expiry sur ingest long) et "build_index_stack
  # KO" (extents incoherents entre scenes, multi-tuile MGRS).
  ...
  # spec 008 sect.5 - La duree est FORMATEE par l'appelant (format_elapsed) avant
  # d'arriver ici : " 13 min 39 s ", plus " 819 s ". D'ou %s et non %.0f.
  monitoring_health_success_done = list(
    fr = "Diagnostic FORDEAD termin\u00e9 en %s.",
    en = "FORDEAD diagnosis completed in %s."
  ),
  # spec 008 sect.3 - Un job tue par le plafond memoire (cgroup MemoryMax pose par
  # le coeur) sort en SIGKILL -> exit 137. Le message brut (" ... failed for zone 3
  # (exit 137) ") ne dit rien a l'utilisateur : on lui substitue une consigne.
  ...
  # v0.106.4 - Bandeau " composition d'essences " RECONFORT, symetrique du
  # bandeau FORDEAD (monitoring_warning_species_*). RECONFORT est calibre sur
  # les FEUILLUS : chene, chataignier, pin sylvestre.
  ...
  # --- Planche pixel deperissement 4 panneaux (Partie B) ---
  ...
  # v0.106.4 - la carte est 100 % raster : l'etat vide ne parle plus d'" alertes "
  # mais de l'absence de RUN (rien a afficher tant qu'aucun diagnostic n'a tourne).
  ...
  # (symetriques avec Alertes FAST/FORDEAD et Carte FAST/FORDEAD).
  # Source figee par onglet, plus de radio a choisir.
  ...
  # Spec control_classes (v0.51.0) - selection des classes servant a
  # tirer les placettes temoins + aide au choix (distribution du raster).
  ...
  # v0.45.0 - labels unifies FAST entre Alertes FAST (legende) et
  # Plan de validation FAST (checkboxes). FAST = quartiles du raster
  # d'alerte courant, calcules dynamiquement via .fast_class_labels().
  # FORDEAD = labels biologiques statiques (le mask categoriel
  # FORDEAD est fixe).
  ...
  # Fallback FAST quand le raster d'alerte n'est pas encore calcule
  # (cache absent, ingestion pas encore lancee, etc.) - labels
  # generiques par classe.
  ...
  # FORDEAD : libelles biologiques fixes (mapping coeur du masque
  # categoriel produit par run_fordead_dieback()).
  ...
  # --- Plan de validation FAST branche sur le trend (spec 025) ----------
  ...
  regen_export_terrain_empty = list(fr = "Aucune UGF \u00e0 envoyer (lancez d'abord l'analyse reG\u00e9n\u00e9ration).", en = "No unit to send (run the regeneration analysis first)."),
  regen_export_terrain_ok_fmt = list(fr = "%d UGF envoy\u00e9es vers le terrain (couche \u00ab regeneration \u00bb).", en = "%d units sent to the field (layer \"regeneration\")."),
  regen_export_terrain_failed = list(fr = "\u00c9chec de l'envoi vers le terrain.", en = "Failed to send to the field."),
  regen_download_pdf = list(fr = "T\u00e9l\u00e9charger le PDF", en = "Download PDF"),
  regen_export_no_quarto = list(fr = "Quarto est requis pour l'export PDF.", en = "Quarto is required for the PDF export."),
  regen_export_pdf_busy = list(fr = "G\u00e9n\u00e9ration du PDF reG\u00e9n\u00e9ration\u2026", en = "Generating the regeneration PDF\u2026"),
  regen_export_pdf_failed = list(fr = "\u00c9chec de la g\u00e9n\u00e9ration du PDF.", en = "PDF generation failed."),
  ...
  regen_ai_panel_help = list(fr = "Conseil narratif bas\u00e9 sur le classement top-3 des essences et les conditions de station. Pose une consigne, choisis la port\u00e9e, envoie.", en = "Narrative advice from the top-3 species ranking and site conditions. Enter an instruction, pick a scope, send."),
  regen_ai_scope = list(fr = "Port\u00e9e", en = "Scope"),
  regen_ai_scope_all = list(fr = "Toutes les UGF du projet", en = "All project UGFs"),
  regen_ai_scope_sel = list(fr = "S\u00e9lection courante", en = "Current selection"),
  regen_ai_replace = list(fr = "Remplacer le conseil pr\u00e9c\u00e9dent", en = "Replace previous advice"),
  regen_ai_placeholder = list(fr = "Ex. : Quelles essences privil\u00e9gier sur les UGF les plus s\u00e9chardes, et avec quel m\u00e9lange ?", en = "E.g.: Which species to favour on the driest UGFs, and with what mixing?"),
  regen_ai_send = list(fr = "Envoyer", en = "Send"),
  regen_ai_clear = list(fr = "Effacer", en = "Clear"),
...
translate_task_message <- function(task, i18n) {
  ...
  # a multi-minute ML run as a "Telechargement".
  ...
  # Open-Canopy pipeline phases (5 steps).
  #   "chm_phase:load_aoi"                   -> "Etape 1/5 : chargement de l'AOI..."
  #   "chm_phase:download_ortho"             -> "Etape 2/5 : telechargement ortho IGN..."
  #   "chm_phase:setup_python"               -> "Etape 3/5 : configuration Python..."
  #   "chm_phase:download_model:pvtv2"       -> "Etape 3/5 : telechargement modele pvtv2..."
  #   "chm_phase:inference:pvtv2"            -> "Etape 4/5 : inference modele pvtv2..."
  #   "chm_phase:export"                     -> "Etape 5/5 : export des resultats..."
  ...
  #   "chm_tile_start:rvb:28"  -> "Telechargement RVB : 28 tuiles..."
  #   "chm_tile:rvb:5:28"      -> "Telechargement RVB : tuile 5/28 ..."
  ...
  #   "chm_inference_start:3"       -> "Inference : 3 tuile(s)..."
  #   "chm_inference_tile:2:3"      -> "Inference tuile 2/3..."
