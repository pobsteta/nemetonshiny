...
TRANSLATIONS <- list(
  ...
  acc_ndp1_lidar_label = list(fr = "NDP 1 : corriger la desserte au LiDAR (tous les moteurs)", en = "NDP 1: correct the road network with LiDAR (all engines)"),
  acc_ndp1_duration_note = list(fr = "Calcul long (~2-3 h sur une desserte r\u00e9elle) : la desserte est recal\u00e9e sur sa vraie position LiDAR, les tron\u00e7ons fant\u00f4mes retir\u00e9s et la largeur mesur\u00e9e \u2014 cette desserte corrig\u00e9e alimente TOUS les moteurs. Fiable (repli automatique sur la desserte brute si la qualification \u00e9choue).", en = "Long computation (~2-3 h on a real road network): the network is re-aligned to its true LiDAR position, phantom segments are removed and width is measured \u2014 this corrected network feeds ALL engines. Reliable (automatic fallback to the raw network if qualification fails)."),
  ...
  acc_desserte_ndp0_badge = list(fr = "NDP 1 demand\u00e9 mais LiDAR indisponible (nuage absent ou foretaccess < 1.19.1) : desserte brute.", en = "NDP 1 requested but LiDAR unavailable (no point cloud or foretaccess < 1.19.1): raw road network."),
  acc_desserte_retires_fmt = list(fr = "%d tron\u00e7on(s) fant\u00f4me(s) retir\u00e9(s).", en = "%d phantom segment(s) removed."),
