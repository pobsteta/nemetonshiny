...
- **Carte interactive** – selection de parcelles cadastrales via Leaflet
...
- **R** \>= 4.1.0
- **nemeton** \>= 0.15.1 (package core)
- Librairies systeme : GDAL, GEOS, PROJ, libudunits2

### Dependances R principales

`shiny`, `bslib`, `leaflet`, `sf`, `terra`, `DT`, `promises`, `ellmer`,
`httr2`, `yaml`, `jsonlite`.
...
`NEMETON_LANG` – langue (defaut : `fr`) - `NEMETON_PAYS` – pays (defaut
: `FR`) - `MISTRAL_API_KEY` – cle API Mistral pour la synthese LLM
